...
class AugeiasView:
    ...
    def update_object(self):
        ...
        res.json_body = {
            ...
            "uri": collection.uri_generator.generate_object_uri(
                collection=collection.name, container=container_key, object=object_key
            ),
        }
        return res
    ...
    def update_file_in_zip(self):
        ...
        zip_content = collection.object_store.get_object(container_key, object_key)
        new_archive = replace_file_in_zip(
            zip_content, file_to_replace, file_content.read(), new_file_name
        )
        collection.object_store.update_object(container_key, object_key, new_archive)
        res = Response(content_type="application/json", status=200)
        res.json_body = {
            "container_key": container_key,
            "object_key": object_key,
    ...
    def get_object_info(self):
        """retrieve object info (mimetype, size, time last modification)
        from the data store"""
        collection = _retrieve_collection(self.request)
        container_key = self.request.matchdict["container_key"]
        object_key = self.request.matchdict["object_key"]
    ...
    def get_container_data(self):
        ...
        res = Response(
            content_type="application/zip", status=200, content_disposition=disposition
        )
    ...
    def create_container(self):
        ...
        res.json_body = {
            ...
            "uri": collection.uri_generator.generate_container_uri(
                collection=collection.name, container=container_key
            ),
        }
        return res
...
def _get_object_data_from_stream(request):
    if "Content-Length" not in request.headers or not _is_integer(
...
def _get_object_data_from_json_body(request):
    ...
    if not collection:
        raise HTTPBadRequest(
            "Collection {} was not found".format(json_data["collection_key"])
        )
    try:
        ...
        raise HTTPBadRequest(
            "Container - object ({} - {}) combination was "
            "not found in Collection {}".format(
                json_data["container_key"],
                json_data["object_key"],
